...
export const HTMLFactory = (
  ...
  const other: Partial<IOptions> = options[mode] || {};

  const mergedMeta = [...(options.common || defaultOptions).meta, ...other.meta ];
  const mergedOptions = { ...(options.common || defaultOptions), ...other };
...
export interface IOptionSet {
  common?: IOptions,
  development?: Partial<IOptions>,
  production?: Partial<IOptions>
}
